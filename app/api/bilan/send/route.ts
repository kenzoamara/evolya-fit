...
export async function POST(req: Request) {
  ...
  const { client_id, coach_note } = await req.json()
  ...
  const { data: client } = await admin
    .from('clients')
    .select('id, full_name, coach_id, weight_kg, main_goal, created_at')
    .eq('id', client_id)
    .eq('coach_id', user.id)
    .single()
  ...
  const [
    ...
    admin.from('checkins').select('id, submitted_at').eq('client_id', client_id).gte('submitted_at', since),
  ...
  if (error) {
    console.error('[bilan/send]', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }

  return NextResponse.json({ ok: true, bilan_id: bilan.id })
}
