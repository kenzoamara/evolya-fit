...
/** Regroupe les exercices par superset pour l'affichage athlète */
function groupClientExercises(exercises: Exercise[]): Array<{ group: string | null; items: Exercise[] }> {
    ...
    return (
      <div className="flex-1 px-4 py-6 max-w-lg mx-auto">
        ...
        <div className="space-y-3">
          {groupClientExercises(data.day.exercises).map(group => {
            if (!group.group) {
                ...
                <div key={ex.id} className="bg-white rounded-2xl border border-[#E2E8F0] p-4">
                  <div className="flex items-center gap-2">
                    ...
                    {ex.library_instructions && (
                      <button onClick={() => setOpenInstructions(prev => ({ ...prev, [ex.id]: !prev[ex.id] }))}
                        className="ml-auto shrink-0 flex items-center gap-1 text-[11px] font-medium text-[#4E9B6F] bg-[#EEF9F3] px-2 py-1 rounded-lg">
                        <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><circle cx="5" cy="5" r="4.5" stroke="#4E9B6F" strokeWidth="1"/><path d="M5 4.5V7M5 3h.01" stroke="#4E9B6F" strokeWidth="1.2" strokeLinecap="round"/></svg>
                        Tuto
                      </button>
                    )}
                  </div>
            ...
            // Superset group
            return (
              <div key={group.group} className="border border-[#4E9B6F]/30 rounded-2xl overflow-hidden">
                <div className="px-4 py-1.5 bg-[#EEF9F3] border-b border-[#4E9B6F]/20">
                  <span className="text-[10px] font-bold text-[#4E9B6F] uppercase tracking-wider">
                    Superset {group.group} · sans repos entre les exercices
                  </span>
                </div>
                <div className="divide-y divide-[#F1F5F9]">
                  {group.items.map(ex => (
                    <div key={ex.id} className="bg-white p-4">
                      <div className="flex items-center gap-2">
                        <span className="w-7 h-7 rounded-lg bg-[#EEF9F3] flex items-center justify-center shrink-0">
                          <Dumbbell size={13} className="text-[#4E9B6F]" />
                        </span>
      ...
      <div className="space-y-4">
        {groupClientExercises(data.day.exercises).map(group => {
          ...
          if (!group.group) {
            const ex = group.items[0]
            return (
              <div key={ex.id} className="bg-white rounded-2xl border border-[#E2E8F0]">
                <ExCard ex={ex} />
              </div>
            )
          }
          // Superset : exercices enchaînés affichés dans un même bloc
          return (
            <div key={group.group} className="border border-[#4E9B6F]/40 rounded-2xl overflow-hidden">
              <div className="px-4 py-1.5 bg-[#EEF9F3] border-b border-[#4E9B6F]/20">
                <span className="text-[10px] font-bold text-[#4E9B6F] uppercase tracking-wider">
              ...
              {group.items.map((ex, i) => (
                <div key={ex.id} className={`bg-white ${i < group.items.length - 1 ? 'border-b border-[#F1F5F9]' : ''}`}>
                  <ExCard ex={ex} />
                </div>
              ))}
            </div>
...
function ProgrammeOverview({
  ...
  // Regrouper par phase si le programme a plusieurs phases, sinon par semaine
  const hasMultiplePhases = new Set(data.days.map(d => d.phase ?? 1)).size > 1

  // Group days by week within each phase
  const phases = Array.from(new Set(data.days.map(d => d.phase ?? 1))).sort((a, b) => a - b)
  ...
  return (
    <div className="flex-1 px-4 py-6 max-w-lg mx-auto">
      ...
      {/* Days — par phase si périodisé, sinon par semaine */}
      <div className="space-y-6">
        {hasMultiplePhases ? phases.map(phase => {
          const phaseDays = data.days.filter(d => (d.phase ?? 1) === phase)
          return (
            <div key={phase}>
              <p className="text-[11px] font-bold text-[#94A3B8] uppercase tracking-wide mb-2">
                Phase {phase} · {phaseDays.length} séance{phaseDays.length !== 1 ? 's' : ''}
              </p>
              <div className="space-y-2">
                {phaseDays.map(day => {
                  ...
                  return (
                    <button key={day.id} onClick={() => onSelectDay(day)}
                      className="w-full flex items-center gap-3 bg-white rounded-2xl border p-4 text-left transition-all active:scale-[0.99]"
                      style={{ borderColor: isToday ? '#4E9B6F' : '#E2E8F0', boxShadow: isToday ? '0 0 0 1px #4E9B6F' : 'none' }}>
                      <div className="shrink-0 w-9 h-9 rounded-xl flex items-center justify-center"
                        style={{ backgroundColor: isDone ? '#EEF9F3' : isToday ? '#0D1F3C' : '#F8FAFB' }}>
                        {isDone ? <CheckCircle2 size={18} className="text-[#4E9B6F]" />
                          : isToday ? <Dumbbell size={16} className="text-white" />
                          : <span className="text-[12px] font-bold text-[#94A3B8]">{day.day_number}</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-[14px] font-semibold text-[#0D1F3C] truncate">{day.title ?? `Jour ${day.day_number}`}</p>
                          {isToday && <span className="shrink-0 text-[10px] font-bold text-white bg-[#4E9B6F] px-2 py-0.5 rounded-full">Aujourd'hui</span>}
                          {isDone && !isToday && <span className="shrink-0 text-[10px] font-semibold text-[#4E9B6F]">Fait</span>}
                        </div>
                        <p className="text-[12px] text-[#94A3B8] mt-0.5">{day.exercise_count} exercice{day.exercise_count !== 1 ? 's' : ''}</p>
                      </div>
