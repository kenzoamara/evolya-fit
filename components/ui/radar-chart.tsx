  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        data-chart={chartId}
        className={cn(
          "[&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted flex aspect-square justify-center text-xs [&_.recharts-layer]:outline-hidden [&_.recharts-surface]:outline-hidden",
          className
        )}
        {...props}
      >
              ...
              <div className="flex flex-1 justify-between items-center leading-none">
                ...
                {item.value && (
                  <span className="text-foreground font-mono font-medium tabular-nums ml-2">
                    {typeof item.value === 'number' ? `${item.value.toFixed(1)} cm` : item.value}
                  </span>
                )}
              </div>
            </div>
