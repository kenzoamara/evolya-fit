...
function WhatsAppSection() {
  const [status, setStatus] = useState<'idle' | 'loading' | 'SCAN_QR_CODE' | 'CONNECTED' | 'error'>('idle')
  ...
  useEffect(() => {
    fetch('/api/whatsapp/status')
      ...
      .then(data => {
        ...
        else if (data.status === 'SCAN_QR_CODE') { setStatus('SCAN_QR_CODE'); setQr(data.qr) }
        else setStatus('idle')
      })
  ...
  async function handleConnect() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { toast.error(data.error ?? 'Erreur connexion'); setStatus('idle'); return }
      setQr(data.qr)
      setStatus('SCAN_QR_CODE')
      startPolling()
    } catch {
  ...
  return (
    <section className="border border-[#E2E8F0] rounded-lg p-5 sm:p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h2 className="text-base font-semibold text-[#0D1F3C]">WhatsApp</h2>
          <p className="text-[12px] text-[#94A3B8]">Envoyer et recevoir des messages via WhatsApp</p>
        </div>
        <div className="ml-auto">
          {status === 'CONNECTED' ? (
            <span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-[#10B981] bg-[#ECFDF5] px-3 py-1 rounded-full">
              <span className="w-1.5 h-1.5 rounded-full bg-[#10B981]" /> Connecté
            </span>
          ) : status === 'SCAN_QR_CODE' ? (
            <span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-[#F59E0B] bg-[#FFF8EC] px-3 py-1 rounded-full">
              <span className="w-1.5 h-1.5 rounded-full bg-[#F59E0B] animate-pulse" /> En attente
            </span>
          ) : null}
        </div>
      </div>
      ...
      {status === 'idle' && (
        <div>
          <p className="text-[13px] text-[#64748B] mb-4 leading-relaxed">
            Connectez votre WhatsApp pour envoyer vos messages directement sur le téléphone de vos élèves. Les réponses arrivent automatiquement dans la messagerie Evolya.
          </p>
          <button
            onClick={handleConnect}
            disabled={connecting}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-semibold text-white transition-colors disabled:opacity-50"
      ...
      {status === 'loading' && (
        <div className="flex items-center gap-3 py-4">
          ...
          <p className="text-[13px] text-[#64748B]">Initialisation de la session…</p>
        </div>
      )}

      {status === 'SCAN_QR_CODE' && (
        <div className="flex flex-col sm:flex-row items-start gap-6">
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-3 shrink-0">
            {qr ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={qr.startsWith('data:') ? qr : `data:image/png;base64,${qr}`} alt="QR Code WhatsApp" width={160} height={160} />
            ) : (
          ...
          <div>
            <p className="text-[13px] font-semibold text-[#0D1F3C] mb-2">Scannez ce QR code</p>
            <ol className="text-[12px] text-[#64748B] space-y-1.5 list-decimal list-inside leading-relaxed">
            ...
            <p className="text-[11px] text-[#94A3B8] mt-3">Le QR se rafraichit automatiquement toutes les 3s</p>
          </div>
        </div>
      )}
      ...
      {status === 'CONNECTED' && (
        <div>
          <p className="text-[13px] text-[#64748B] mb-4">
            Votre WhatsApp est connecté. Les messages envoyés depuis Evolya seront aussi envoyés sur WhatsApp, et les réponses de vos élèves arriveront directement ici.
          </p>
          <p className="text-[12px] text-[#94A3B8] mb-4">
            Pour activer WhatsApp pour un élève, ajoutez son numéro de téléphone dans sa fiche client.
          </p>
          <button
            onClick={handleDisconnect}
            disabled={disconnecting}
            className="inline-flex items-center gap-2 px-4 py-2 border border-[#E2E8F0] text-[#64748B] rounded-xl text-[13px] font-medium hover:border-red-300 hover:text-red-500 transition-colors disabled:opacity-50"
          >
            {disconnecting ? <span className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" /> : null}
            Déconnecter
          </button>
        </div>
      )}
